...
class GaussianProcessTemplateWeighter(gpytorch.models.ApproximateGP):
    ...
    def __init__(self, text_embeddings: torch.Tensor, cfg: Any, **kwargs) -> None:
        ...
        self.num_mc_samples_train = self.get_config_value(cfg, 'GP_NUM_MC_SAMPLES_TRAIN', 30)
        self.num_mc_samples_eval = self.get_config_value(cfg, 'GP_NUM_MC_SAMPLES_EVAL', 30)
        ...
        kernel_type = self.get_config_value(cfg, 'GP_KERNEL_TYPE', 'rbf').lower()
        if kernel_type == "rbf":
            with torch.no_grad():
    ...
    def sample_prototypes(self, num_samples: int, visual_embeddings: torch.Tensor = None) -> torch.Tensor:
        ...
        if visual_embeddings.shape[0] == self.num_classes:
            ...
            gp_input = torch.cat([self._templates, visual_token], dim=1)  # [K, M+1, D]
