...
#         if not hasattr(cfg.adapter, 'gp_pca_dims') or cfg.adapter.gp_pca_dims is None:
#             text_embeddings_reduced = text_embeddings_fp32
#         else:
#             text_embeddings_reduced = self._reduce_inducing_dims(text_embeddings_fp32, cfg)

#         # self.num_classes, self.num_templates, self.dim = text_embeddings_fp32.shape
#         self.num_classes, self.num_templates, self.dim = text_embeddings_reduced.shape
...
#         inducing_inputs = text_embeddings_reduced.detach().clone().view(
...
#             inducing_inputs,
#             variational_dist,
#             learn_inducing_locations=cfg.adapter.gp_learn_inducing,
#         )
...
#         with torch.no_grad():
#             class_mean = text_embeddings_reduced.mean(dim=1, keepdim=True)  # [K,1,D]
#             text_norm = F.normalize(text_embeddings_reduced, p=2, dim=-1)   # [K,M,D]
...
#         if kernel_type == "rbf":
#             with torch.no_grad():
#                 flat_emb = F.normalize(text_embeddings_reduced.reshape(-1, self.dim), p=2, dim=-1)  # [(K*M), D]
...
#         self.register_buffer("_templates_reduced", text_embeddings_reduced.detach())

#     def _reduce_inducing_dims(self, X, cfg):
#         K, M, D = X.shape
#         x_np = X.reshape(-1, D).cpu().numpy()
#         x_pca = PCA(n_components=cfg.adapter.gp_pca_dims).fit_transform(x_np)
#         x_pca = x_pca.reshape(K, M, -1)
#         return torch.from_numpy(x_pca).to(device=X.device, dtype=X.dtype)
...
#         qf = self(self._templates_reduced.to(torch.float32))
...
#         except Exception:
#             pass
#         f_dist = self(self._templates_reduced.to(torch.float32))
...
class GaussianProcessTemplateWeighter(gpytorch.models.ApproximateGP):
    ...
    def __init__(self, text_embeddings: torch.Tensor, cfg: Any, **kwargs) -> None:
        ...
        with torch.no_grad():
            cls_mean = text_embeddings_reduced.mean(dim=1, keepdim=True)  # [K,1,D]
        ...
        # Use the template encodings as inducing points.
        inducing_inputs = text_embeddings_reduced.detach().clone().view(self.num_classes, self.num_templates, self.dim)
        inducing_inputs = torch.cat([inducing_inputs, cls_mean], dim=1)  # [K, M+1, D]


        variational_dist = gpytorch.variational.CholeskyVariationalDistribution(num_inducing_points=self.num_templates+1,
                                                                                batch_shape=batch_shape,)

        # variational_strategy = gpytorch.variational.VariationalStrategy(self, inducing_inputs, variational_dist, learn_inducing_locations=cfg.adapter.gp_learn_inducing)
        ...
        # Freeze: GP learns only residuals around the zero-shot prior
        # self.mean_module.mean_param.requires_grad_(False)
    ...
    def sample_prototypes(self, num_samples: int, visual_embeddings: torch.Tensor) -> torch.Tensor:
        """
        Draw *num_samples* sets of template-weighted class prototypes.
        Returns
        -------
        ...
        N_templates = self._templates_reduced.shape[1]
        if visual_embeddings.shape[0] == self.num_classes:
            visual_embeddings = visual_embeddings.unsqueeze(1)
            gp_input = torch.cat([self._templates_reduced, visual_embeddings], dim=1)  # [K, M+1, D]
        else:
            ...
            gp_input = self._templates_reduced  # [K, M, D]
            
        # Distribution over latent function values at template inputs
        # qf = self(self._templates_reduced.to(torch.float32))
        qf = self(gp_input.to(torch.float32))
        # Stochastic sampling of latent function values
...
# base_means = [gpytorch.means.ConstantMean() for _ in range(num_templates)]
# mean_module = gpytorch.means.MultitaskMean(
#     gpytorch.means.MeanList(*base_means), num_tasks=num_templates
# )

# class HierMean(gpytorch.means.Mean):
#     def __init__(self, num_classes, num_templates, d_cls=16, d_tmp=16):
#         super().__init__()
#         self.global_bias = torch.nn.Parameter(torch.zeros(1))
#         self.cls_emb = torch.nn.Embedding(num_classes, d_cls)
#         self.tmp_emb = torch.nn.Embedding(num_templates, d_tmp)
#         self.head = torch.nn.Linear(d_cls + d_tmp, 1, bias=False)

#     def forward(self, x):
#         # x carries (class_id, template_id) per row
#         cls_id, tmp_id = x[:,0].long(), x[:,1].long()
#         h = torch.cat([self.cls_emb(cls_id), self.tmp_emb(tmp_id)], dim=-1)
#         return self.global_bias + self.head(h).squeeze(-1)
