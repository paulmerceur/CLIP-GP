...
def make_plots(grouped, exp_name: str):
    ...
    for ds, shots_map in grouped.items():
        ...
        for cfg, shot_map in per_cfg.items():
            ...
            ax_acc.plot(xs, ys_acc, marker="o", label=cfg)
            ax_ece.plot(xs, ys_ece, marker="o", label=cfg)
            ax_aece.plot(xs, ys_aece, marker="o", label=cfg)
        ...
        if labels:
            fig.legend(handles, labels, loc="upper center", bbox_to_anchor=(0.5, 0.95), ncol=min(4, len(labels)), frameon=False)
